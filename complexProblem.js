...
the positive integers (decimal numbers are not integers) when an array of real numbers is passed to it.
An example of an array of real numbers is [-3, 4.8, 5, 3, -3.2].
Note: Your function should not use any kind of for or while loops or the forEach() function.
*/

const squareList = (arr) => {
    let newArr = arr.filter(n => n % parseInt(n) === 0 && n > 0)
    .map(n => Math.pow(n, 2));
    return newArr;
};

const squaredIntegers = squareList([-3, 4.8, 13, 12, -3.2]);
